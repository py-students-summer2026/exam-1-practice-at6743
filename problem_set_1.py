...
def bark():
    ...
    """
    # asking user for name and putting it in capital case
    name = input("What is the name of your dog ? ").capitalize()

    # asking age of user's dog
    age = input("How old is your dog ?")

    # asking user for breed and putting it in capital case
    breed = input("What is the breed of your dog ? ").capitalize()
...
def bark_with_validation():
    ...
    elif not age.isdigit() or int(age) < 0 or int(age) > 15 : print("Age error!")

    elif breed not in ["Schnauzer", "Terrier", "Poodle", "Mastiff"] : 
        print("Breed error!")


    else : print(f'{name}, the {age} year old {breed}, says, "Woof!"')
...
def respond_to_anything():
    ...
    else : print("Please include a punctuation mark at the end of your sentence!")
...
def respond_to_anything_but_nonsense():
    ...
    if "nonsense" in sentence.lower() : # must be first since first check is to see if nothing must be print, then moves to commands to print something.
        print("")
